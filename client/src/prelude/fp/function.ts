...
import { flow, FunctionN, identity, Predicate, Refinement } from 'fp-ts/lib/function'
...
>(
    ...
    p5: Refinement<A, B5>,
    p6: Refinement<A, B6>
): Refinement<A, B1 | B2 | B3 | B4 | B5 | B6>
export function or<
    A,
    B1 extends A,
    B2 extends A,
...
export function composeRefinements<A, B extends A, C extends B, D extends C>(
    pab: Refinement<A, B>,
    pbc: Refinement<B, C>,
    pcd: Refinement<C, D>
): Refinement<A, D>
export function composeRefinements(
    pab: Predicate<unknown>,
    pbc: Predicate<unknown>,
    pcd: Predicate<unknown>
): Predicate<unknown> {
    return (a: unknown) => pab(a) && pbc(a) && pcd(a)
}
...
export const curry = <A, B, C>(f: (a: A, b: B) => C): Curried<[A, B], C> => a => b => f(a, b)

export const curryAndFlip = flow(curry, flipC)